...
//load the Landsat-8 image collection, filter by specifications
var l8_original = ee.ImageCollection("LANDSAT/LC08/C02/T1_L2")
...
//apply cloud mask
var l8_masked = l8_original.map(maskL8sr);
...
var l8_masked = ee.ImageCollection(l8_masked)
  .select(
...
//apply resampling
var l8_masked = l8_masked.map(resampleLandsat);
...
var l8_masked = l8_masked.map(addIndices);
...
var region_buf = region.toList(region.size());
var forExport = region_buf.map(function (ele) {
  var l8_masked_export = l8_masked.filter(ee.Filter.bounds(ee.Feature(ele).geometry()));
  var getData = l8_masked_export.toList(l8_masked_export.size()).map(function (e) {
...
Export.table.toDrive({
    ...
    description: 'L8_2017_2023', //define a name for the CSV
